...
db.MantenimientoTemp.aggregate([ { \$addFields: { fechaIntervencion: { \$dateFromString: { dateString: "\$fechaIntervencion", format: "%d-%m-%Y" } } } }, { \$project: { _id: 0 } }, {\$project: {_id: 0}}, { \$merge: { into: "Mantenimiento", on: "id", whenMatched: "merge" } } ]);
